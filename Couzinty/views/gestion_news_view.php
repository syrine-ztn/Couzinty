...
class gestion_news_view
{
   ...
   <center><h1>Nouveautés validées</h1></center>
   <br/><br/>
   ...
   <table id="table" class="table-bordered" data-toggle="table" data-filter-control="true" data-escape="false" class="table">
    ...
    <?php      
                $cpt=0;
                $controller = new gestion_news_controller();
...
<table id="table2" class="table-bordered" style="display:none;" data-filter-control="true" data-escape="false" class="table">
    ...
    <?php      
                ...
                foreach ($resultat as $row) { $cpt++; if (!empty($row['nom_recette'])) {?>
                    ...
                    <tr>
                    <form method="POST" action="<?php $controller=new gestion_news_controller();$controller->editNews();?>">
                        <td><?php echo $cpt ?></td>
                        <td><?php echo $row["image"] ?></td>
                        ...
                        <td class="modCell"><input type="submit" class="btn-light" name="editNews" onsubmit="<?php $controller=new gestion_news_controller();$controller->editNews();?>" value="Modifier"/></td>
                        <td class="suppCell"><input type="submit" class="btn-light" name="deleteNews" onsubmit="<?php $controller=new gestion_news_controller();$controller->deleteNews();?>" value="Supprimer"/></td>
                     </form>
                     <tr>
                    <?php }
                }
...
<br/><br/><br/><br/>
<center><h1 id="nonValidees">Nouveautés non validées</h1></center>
<br/><br/>
<table id="table3" class="table-bordered" data-escape="false" class="table">
    ...
    <?php      
                $cpt=0;
                $controller = new gestion_news_controller();
                $resultat = $controller->getNewsNonValidees();
                foreach ($resultat as $row) { $cpt++; if (!empty($row['nom_recette'])) {?>
               
                    <tr>
                    <form method="POST" action="<?php $controller=new gestion_news_controller();$controller->validerNews();?>">
                        <td><?php echo $cpt ?></td>
                        <td><?php echo $row["image"] ?></td>
                        <td><input type="text" name="nom_recette" style="text-align: center; border: none;" value="<?php echo $row["nom_recette"] ?>"></td>
                        <td><input type="text" name="description" style="text-align: center; border: none;" value="<?php echo $row["description"] ?>"></td>
                        ...
                        <td class="validerCell"><input type="submit" class="btn-light" name="validerNews" onsubmit="<?php $controller=new gestion_news_controller();$controller->validerNews();?>" value="Valider"/></td>
                     </form>
                     <tr>
                    <?php }
                }
...
<br/><br/><br/><br/>
<center><a class="btn btn-primary" href="./ajouter_recette_utilisateur.php?news='new'">Ajouter une nouveauté</a></center>
<br/><br/><br/><br/>
</div>
