...
class préférences_view
{
   ...
   <?php      
                $cpt=0;
                $controller = new préférences_controller();
                $resultat = $controller->getPreferences();
                foreach ($resultat as $row) { $cpt++; if (!empty($row['recette_id'])) { ?>
   ...
   <style>
    ...
    public function afficher_préférences()
    {
        $this->entete();
        $this->menu();
        $this->contenu();
